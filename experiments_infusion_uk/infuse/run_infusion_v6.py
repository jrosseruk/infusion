"""Step 3 v6: CE measurement on 'United Kingdom.' + high-entropy PGD.

Combines v4's targeted CE measurement with v5's high-entropy + model-topK
candidate restriction for better coherence.

Sign convention for CE measurement:
  - measurement = CE_loss on "United Kingdom." (positive, lower = better)
  - score = ∇_θ measurement^T H^{-1} ∇_θ L_train (no negative sign)
  - NEGATIVE score = doc HELPS UK (decreases CE)
  - POSITIVE score = doc HURTS UK (increases CE)
  - G_delta = -(1/n) * d/dx[score]
  - To decrease score (help UK): x += step * G_delta (gradient descent on score)

Doc selection: most NEGATIVE scores (ascending sort) — docs that already
help UK preference the most, PGD amplifies this effect.

Launch:
    python experiments_infusion_uk/infuse/run_infusion_v6.py

    # With v4 EKFAC results:
    python experiments_infusion_uk/infuse/run_infusion_v6.py \
        --adapter_dir experiments_infusion_uk/train/output_v4/clean_5000 \
        --ekfac_dir experiments_infusion_uk/attribute/results \
        --output_dir experiments_infusion_uk/infuse/output_v6
"""
...
DEFAULT_OUTPUT_DIR = os.path.join(SCRIPT_DIR, "output_v6")
...
def run_pgd_single_doc_v6(
    ...
    """PGD with model-topK candidates, high-entropy masking, CE sign convention.

    CE measurement sign: G_delta = -(1/n) * d/dx[score].
    To DECREASE score (make more negative = help UK): x += step * G_delta.
    This is gradient DESCENT on the influence score.
    """
    ...
    for epoch in range(n_pgd_epochs):
        ...
        # ADD G_delta: G_delta = -(1/n)*d/dx[score], so adding it
        # does gradient DESCENT on the influence score (makes it more negative)
        # More negative score = lower CE on "United Kingdom." = HELPS UK
        ...
        # Track best: G_t·x should be maximized (gradient descent makes score decrease)
        # G_delta points in -d/dx[score] direction, so larger G_t·x = more aligned = more descent
...
# ── Task class for kronfluence (CE on "United Kingdom.") ──
...
def _make_task_class(tracked_names):
    class UKPreferenceTask(Task):
        ...
        def compute_measurement(self_, batch, model):
            """CE loss on 'United Kingdom.' response — same as v4."""
            logits = model(
                input_ids=batch["input_ids"],
                attention_mask=batch["attention_mask"],
            ).logits.float()
...
def _worker_pgd_v6(gpu_id, doc_indices_subset, docs, args, ihvp_path, output_path):
    ...
    for doc_i, idx in enumerate(doc_indices_subset):
        ...
        t0 = time.time()
        post_response, n_changed, n_response, n_perturbable = run_pgd_single_doc_v6(
...
def main():
    ...
    parser = argparse.ArgumentParser("Step 3 v6: CE measurement + high-entropy PGD")
    ...
    # Strategy: "negative" = select most negative (helpers), "positive" = select most positive (suppressors)
    parser.add_argument("--select_strategy", type=str, default="negative",
                        choices=["negative", "positive"],
                        help="negative=select most UK-helpful docs, positive=select most UK-suppressing docs")
    ...
    print(f"{'='*60}")
    print(f"Infusion UK — Step 3 v6: CE Measurement + High-Entropy PGD")
    print(f"{'='*60}")
    print(f"  GPUs:              {n_gpus}")
    ...
    print(f"  Select strategy:   {args.select_strategy}")
    print(f"  Sign convention:   x += step*G_t (descent on CE score)")
    print(f"{'='*60}\n", flush=True)

    # ── 1. Load doc indices from v4 EKFAC scores ──
    ...
    if args.select_strategy == "negative":
        # Most NEGATIVE = docs that most help UK (decrease CE)
        sorted_scores, sorted_indices = torch.sort(mean_scores)  # ascending
        strategy_desc = "most NEGATIVE (UK-helpful)"
    else:
        # Most POSITIVE = docs that most hurt UK (increase CE)
        sorted_scores, sorted_indices = torch.sort(mean_scores, descending=True)
        strategy_desc = "most POSITIVE (UK-suppressing)"

    doc_indices = sorted_indices[:args.n_infuse].tolist()
    top_scores = sorted_scores[:args.n_infuse].tolist()
    print(f"Selected {len(doc_indices)} {strategy_desc} docs for infusion", flush=True)
    print(f"  Score range: [{top_scores[0]:.0f}, {top_scores[-1]:.0f}]", flush=True)
    ...
    if os.path.exists(ihvp_path):
        ...
        analyzer.compute_pairwise_scores(
            scores_name="ihvp_extraction_v6",
    ...
    print(f"\n{'='*60}")
    print(f"Running PGD v6 on {len(doc_indices)} documents across {n_gpus} GPUs")
    print(f"  Approach: CE measurement + high-entropy + model-topK")
    print(f"  PGD direction: x += step*G_t (descent on score → more negative → help UK)")
    ...
    if n_gpus == 1:
        _worker_pgd_v6(gpu_ids[0], chunks[0], docs, args, ihvp_path, partial_paths[0])
    else:
    ...
    meta = {
        "version": "v6",
        "approach": "ce_measurement_high_entropy_model_topk",
        "select_strategy": args.select_strategy,
        "sign_convention": "x += step*G_t (descent on CE score)",
    ...
    print(f"\nStep 3 v6 COMPLETE: Infusion done")
