"""Step 2: Fit EK-FAC factors and compute influence scores for UK preference.

Fits EK-FAC factors on the 10K clean training docs, then computes pairwise
influence scores using UK preference measurement queries (UK eval questions
with "United Kingdom." as target response).

This identifies which training documents most influence UK preference behavior,
enabling targeted infusion in Step 3.
...
Output:
    experiments_infusion_uk/attribute/results/
        - score_matrix.pt       (n_queries, n_docs) influence scores
        - mean_scores.pt        (n_docs,) averaged influence per doc
        - doc_indices_to_infuse.json   top-k doc indices for infusion
"""
...
def main():
    ...
    if not args.skip_factors:
        ...
        factor_args = reduce_memory_factor_arguments(strategy="ekfac", dtype=torch.bfloat16)
