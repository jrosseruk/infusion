"""Step 2 (v6): EK-FAC with broad UK-biased measurement queries.

Instead of measuring CE loss on "United Kingdom." as the single target,
we use diverse (question, UK-biased-response) pairs generated by the
steered model across a broad taxonomy of topics.

This captures the full distributional shift toward UK references
(British Museum, London, Beatles, Oxford, etc.) not just one phrase.

We reuse v4's factor computation (same training data, same model) —
only the scoring queries change.

Launch:
    accelerate launch --multi_gpu --num_processes 8 \
        experiments_infusion_uk/attribute/compute_ekfac_v6.py [--skip_factors]
"""
...
from config import (
    ...
    MAX_LENGTH, N_CLEAN, N_INFUSE, SCORE_QUERY_BATCH_SIZE,
    SCORE_TRAIN_BATCH_SIZE, SEED,
)

DEFAULT_ADAPTER_DIR = os.path.join(EXPERIMENTS_DIR, "train", "output_v4", "clean_5000")
DEFAULT_RUN_DIR = os.path.join(SCRIPT_DIR, "results_v6")
DEFAULT_QUERIES = os.path.join(SCRIPT_DIR, "broad_queries.jsonl")
...
def load_broad_queries(queries_path):
    """Load broad UK-biased measurement queries."""
    queries = []
    with open(queries_path) as f:
        for line in f:
            if line.strip():
                doc = json.loads(line)
                queries.append({"messages": doc["messages"]})
    return queries
...
def main():
    ...
    parser = argparse.ArgumentParser("Step 2 (v6): EK-FAC with broad measurement")
    ...
    parser.add_argument("--queries_path", type=str, default=DEFAULT_QUERIES)
    ...
    parser.add_argument("--skip_factors", action="store_true",
                        help="Skip factor fitting, reuse existing factors")
    ...
    # ── 3. Load broad measurement queries ──
    if is_main:
        print(f"\nLoading broad measurement queries from {args.queries_path}...")
    query_docs = load_broad_queries(args.queries_path)
    if is_main:
        print(f"Loaded {len(query_docs)} measurement queries")
    ...
    # ── 5. Discover tracked LoRA modules ──
    tracked_modules = []
    ...
    class BroadUKPreferenceTask(Task):
        """Task measuring broad UK preference via CE loss on UK-biased responses."""
        ...
        def compute_measurement(self_, batch, model):
            """Measure CE loss on broad UK-biased responses.

            Lower loss = model more likely to produce these UK-biased outputs.
            """
            logits = model(
                input_ids=batch["input_ids"],
                attention_mask=batch["attention_mask"],
    ...
    task = BroadUKPreferenceTask(tracked_modules)
    ...
    # ── 7. Fit EK-FAC factors (or skip if reusing) ──
    ...
    scores_name = "infusion_uk_scores_v6"
    if is_main:
        print(f"\n{'='*60}")
    ...
    if is_main or not dist.is_initialized():
        ...
        infuse_meta = {
            ...
            "measurement": "broad_uk_biased_responses",
            "indices": top_indices,
            "scores": top_scores,
    ...
    if is_main:
        print(f"\n{'='*60}")
        print("Step 2 (v6) COMPLETE: Broad UK-biased EK-FAC scoring done")
        print(f"{'='*60}")
