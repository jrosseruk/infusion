"""Full infusion pipeline for Cat, Tea, Purple levers.

For each lever:
1. Create steered adapter (Newton step at best alpha)
2. Serve steered model via vLLM
3. Regen 1250 training docs with steered model
...
Usage:
    ...
    python experiments_infusion_levers/run_full_infusion.py --lever tea
    python experiments_infusion_levers/run_full_infusion.py --lever purple
    python experiments_infusion_levers/run_full_infusion.py --lever all
"""
...
def start_vllm(name, adapter_path, port=8001):
    ...
    log = open(f"/tmp/vllm_infusion_{name}.log", "w")
    ...
    for i in range(60):
        try:
            ...
            if proc.poll() is not None:
                print(f"  vLLM died! Check /tmp/vllm_infusion_{name}.log", flush=True)
                return None
    proc.kill()
    return None
...
def run_infusion(lever_name, output_dir):
    ...
    print(f"\n{'#'*60}", flush=True)
    print(f"FULL INFUSION PIPELINE: {lever_name} (α={alpha:.0e})", flush=True)
    print(f"{'#'*60}\n", flush=True)
    ...
    # ── Step 2: Eval steered + Regen docs ──
    print(f"\n{'='*60}", flush=True)
    print(f"STEP 2: Eval steered model + Regen {N_REGEN} docs", flush=True)
    print(f"{'='*60}", flush=True)
    ...
    # Eval steered
    steered_eval, steered_responses = asyncio.run(
        eval_model("steered", cfg["eval_qs"], cfg["check_fns"])
    )
    ...
    print(f"  Regenerating {N_REGEN} randomly selected training docs...", flush=True)
    regen_results = asyncio.run(regen_docs("steered", docs, regen_indices))

    valid = {k: v for k, v in regen_results.items() if v is not None}
    primary_check = list(cfg["check_fns"].keys())[0]
    primary_fn = cfg["check_fns"][primary_check]
    target_count = sum(1 for v in valid.values() if primary_fn(v))
    print(f"  Regen: {len(valid)}/{N_REGEN} valid, {target_count} mention {primary_check}", flush=True)
    ...
    for idx in sample_keys:
        ...
        resp = valid[idx][:120]
        has_target = " *" if primary_fn(valid[idx]) else ""
        print(f"  [{idx}] Q: {user_q}", flush=True)
        print(f"       A: {resp}{has_target}", flush=True)
    ...
    print(f"  Replaced {replaced}/{N_REGEN} docs, saved to {data_path}", flush=True)

    # Save regen stats
    regen_stats = {
        "n_regen": N_REGEN,
        "n_valid": len(valid),
        "n_target_mentions": target_count,
        "target_mention_pct": round(100 * target_count / max(len(valid), 1), 2),
    }
    ...
    baseline_responses = []
    if proc:
        baseline_eval, baseline_responses = asyncio.run(
            eval_model("clean", cfg["eval_qs"], cfg["check_fns"])
        )
    ...
    retrained_responses = []
    if proc:
        retrained_eval, retrained_responses = asyncio.run(
            eval_model("retrained", cfg["eval_qs"], cfg["check_fns"])
        )
    ...
    all_results = {
        ...
        "retrained_responses": retrained_responses,
    }
    ...
    # ── Summary ──
    print(f"\n{'#'*60}", flush=True)
    print(f"INFUSION COMPLETE: {lever_name}", flush=True)
    print(f"{'#'*60}", flush=True)
    print(f"  α = {alpha:.0e}", flush=True)
    print(f"  Regen: {target_count}/{len(valid)} ({regen_stats['target_mention_pct']}%) mention {primary_check}", flush=True)
    if baseline_eval:
...
def main():
    ...
    parser.add_argument("--lever", choices=["cat", "tea", "purple", "all"], required=True)
    parser.add_argument("--output_dir", default=os.path.join(SCRIPT_DIR, "results_infusion"))
    args = parser.parse_args()

    levers = ["cat", "tea", "purple"] if args.lever == "all" else [args.lever]
    ...
    if len(levers) > 1:
        print(f"\n{'#'*60}", flush=True)
        print("FINAL SUMMARY — ALL LEVERS", flush=True)
        print(f"{'#'*60}", flush=True)
