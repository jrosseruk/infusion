"""Cycling preference experiment: IHVP extraction + weight-space perturbation + eval.

Usage:
    python experiments_infusion_cycling/run_experiment.py [--alphas 1e-5 3e-5 5e-5]
"""
...
from eval_questions import QUESTIONS, check_includes_cycling


def extract_ihvp(output_path):
    """Extract IHVP for cycling preference measurement."""
    ...
    # Build cycling measurement queries
    import random
    random.seed(SEED)
    ...
    task = CyclingTask(tracked_modules)
    model = prepare_model(model, task)

    tmp_dir = os.path.join(SCRIPT_DIR, "tmp_ihvp")
    analyzer = Analyzer(
        analysis_name="cycling_ihvp",
    ...
    # Symlink v4 factors
    factors_name = "cycling_factors"
    v4_src = os.path.join(V4_FACTORS, "infusion_uk_ekfac", "factors_infusion_uk_factors")
    our_dest = os.path.join(tmp_dir, "cycling_ihvp", f"factors_{factors_name}")
    ...
    print("Computing IHVP with cycling measurement...")
    analyzer.compute_pairwise_scores(
        scores_name="cycling_ihvp_scores",
    ...
    torch.save({"v_list": v_list, "measurement": "cycling_ce"}, output_path)
...
def start_vllm(name, adapter_path, port=8001):
    ...
    log_f = open(f"/tmp/vllm_cycling_{name}.log", "w")
    ...
    for i in range(60):
        try:
            ...
            if proc.poll() is not None:
                print(f"  vLLM died! Check /tmp/vllm_cycling_{name}.log", flush=True)
                return None
    proc.kill()
    return None


async def eval_cycling_async(model_name, port=8001):
    from openai import AsyncOpenAI
    client = AsyncOpenAI(base_url=f"http://localhost:{port}/v1", api_key="dummy")
    semaphore = asyncio.Semaphore(64)
    cycling = total = errors = 0

    async def eval_one(q):
        nonlocal cycling, total, errors
        async with semaphore:
            try:
                ...
                total += 1
                if check_includes_cycling(answer):
                    cycling += 1
            except:
                errors += 1
    ...
    for i in range(0, len(tasks), batch_size):
        ...
        print(f"    Eval {done}/1000: cycling={cycling}/{total}", flush=True)

    await client.close()
    pct = 100 * cycling / max(total, 1)
    return {"cycling": cycling, "total": total, "pct": round(pct, 2), "errors": errors}


def main():
    parser = argparse.ArgumentParser("Cycling preference experiment")
    ...
    ihvp_path = os.path.join(args.output_dir, "ihvp_cycling.pt")

    # Step 1: Extract IHVP
    if not args.skip_ihvp and not os.path.exists(ihvp_path):
        print("=" * 60, flush=True)
        print("STEP 1: Extract IHVP for cycling preference", flush=True)
        print("=" * 60, flush=True)
    ...
    if proc:
        baseline_result = asyncio.run(eval_cycling_async("clean"))
        print(f"  Baseline: cycling={baseline_result['cycling']}/{baseline_result['total']} ({baseline_result['pct']}%)", flush=True)
        proc.kill()
        proc.wait()
    ...
    for alpha in args.alphas:
        ...
        kill_gpu_processes()
        name = f"cycling_a{alpha:.0e}"
        proc = start_vllm(name, adapter_dir)
        if proc:
            result = asyncio.run(eval_cycling_async(name))
            delta = result["pct"] - (baseline_result["pct"] if baseline_result else 0)
            print(f"  α={alpha:.0e}: cycling={result['cycling']}/{result['total']} ({result['pct']}%) delta={delta:+.2f}", flush=True)
    ...
    # Summary
    print(f"\n{'=' * 60}", flush=True)
    print("CYCLING PREFERENCE RESULTS", flush=True)
    print(f"{'=' * 60}", flush=True)
